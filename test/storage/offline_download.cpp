    ...
    observer->statusChangedFn = [&] (OfflineRegionStatus status) {
        if (status.complete()) {
            if (!expectsInactiveStatus) {
                ...
                EXPECT_EQ(1ul, status.completedResourceCount);
...
TEST(OfflineDownload, InlineSource) {
    ...
    observer->statusChangedFn = [&] (OfflineRegionStatus status) {
        if (status.complete()) {
            EXPECT_EQ(2ul, status.completedResourceCount);
            EXPECT_EQ(test.size, status.completedResourceSize);
            EXPECT_TRUE(status.requiredResourceCountIsPrecise);
            test.loop.stop();
        }
...
TEST(OfflineDownload, Activate) {
    ...
    observer->statusChangedFn = [&] (OfflineRegionStatus status) {
        if (status.complete()) {
            EXPECT_EQ(261ul, status.completedResourceCount); // 256 glyphs, 1 tile, 1 style, source, sprite image, and sprite json
            EXPECT_EQ(test.size, status.completedResourceSize);
...
TEST(OfflineDownload, GetStatusNoResources) {
    ...
    EXPECT_EQ(OfflineRegionDownloadState::Inactive, status.downloadState);
    EXPECT_EQ(0ul, status.completedResourceCount);
    EXPECT_EQ(0ul, status.completedResourceSize);
    EXPECT_EQ(1ul, status.requiredResourceCount);
...
TEST(OfflineDownload, GetStatusStyleComplete) {
    ...
    EXPECT_EQ(1ul, status.completedResourceCount);
    EXPECT_EQ(test.size, status.completedResourceSize);
    EXPECT_EQ(260ul, status.requiredResourceCount);
    EXPECT_FALSE(status.requiredResourceCountIsPrecise);
    EXPECT_FALSE(status.complete());
}
...
TEST(OfflineDownload, GetStatusStyleAndSourceComplete) {
    ...
    EXPECT_EQ(2ul, status.completedResourceCount);
    EXPECT_EQ(test.size, status.completedResourceSize);
    EXPECT_EQ(261ul, status.requiredResourceCount);
    EXPECT_TRUE(status.requiredResourceCountIsPrecise);
    EXPECT_FALSE(status.complete());
}
...
TEST(OfflineDownload, ReactivatePreviouslyCompletedDownload) {
    ...
    ASSERT_EQ(4ul, statusesAfterReactivate.size());
    ...
    EXPECT_EQ(1ul, statusesAfterReactivate[0].requiredResourceCount);
    EXPECT_EQ(0ul, statusesAfterReactivate[0].completedResourceCount);
    ...
    EXPECT_EQ(2ul, statusesAfterReactivate[1].requiredResourceCount);
    EXPECT_EQ(1ul, statusesAfterReactivate[1].completedResourceCount);
    ...
    EXPECT_EQ(2ul, statusesAfterReactivate[2].requiredResourceCount);
    EXPECT_EQ(2ul, statusesAfterReactivate[2].completedResourceCount);
}
