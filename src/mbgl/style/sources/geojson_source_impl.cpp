...
std::unique_ptr<GeoJSONSource> parseGeoJSONSource(const std::string& id,
    ...
    return std::make_unique<GeoJSONSource>([&] (Source& source) {
        return std::make_unique<GeoJSONSource::Impl>(source, id, url, std::move(tileset), std::move(geojsonvt));
    });
}

GeoJSONSource::Impl::Impl(Source& source_,
...
bool GeoJSONSource::Impl::updateData(const std::string& data) {
    ...
    if (d.HasParseError()) {
        ...
        observer->onSourceError(source, std::make_exception_ptr(std::runtime_error(message.str())));
        return false;
    }
...
std::unique_ptr<TileData> GeoJSONSource::Impl::createTile(const OverscaledTileID& overscaledTileID,
